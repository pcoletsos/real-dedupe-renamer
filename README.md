...
## Latest build
- Windows executable (no Python needed): `dist/delete_real_duplicates_v1.1.0.exe`  
  Built from the current codebase (PyInstaller one-file, windowed). Run it directly or share the EXE.

## Run options
1) **Executable:** double-click `dist/delete_real_duplicates_v1.1.0.exe` (or run from a terminal).  
   - You can also share `release-delete-real-duplicates-v1.1.0.zip`, which contains the EXE.
2) **Source (Python 3.9+):**
   ```bash
...
- Settings persistence: last folder, days, criteria, hash limit, and same-folder toggle saved to `.duplicate_cleaner_settings.json`.
