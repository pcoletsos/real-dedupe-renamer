...
## Latest build
- Windows executable (no Python needed): `dist/delete_real_duplicates-1.7.8.exe`  
  Built from the current codebase (PyInstaller one-file, windowed). Run it directly or share the EXE.

## Run options
1) **Executable:** double-click `dist/delete_real_duplicates-1.7.8.exe` (or run from a terminal).
2) **Source (Python 3.9+):**
   ```bash
